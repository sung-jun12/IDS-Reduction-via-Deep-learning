...
def segmentation_indel_reduction(model, model_type, noisy_img, noisy_seq, period=16, device='cpu'):
    ...
    mask_input = run_segmentation(noisy_img)
    flat_input = mask_input.reshape(-1)
    try:
        pos_input = next(i for i, v in enumerate(flat_input) if v != 0)
        err_type = int(flat_input[pos_input])
    except StopIteration:
        return noisy_img, noisy_seq

    dna_chunks = [noisy_seq[i:i+4] for i in range(0, len(noisy_seq), 4)]
    ...
    pos_A = first_error_pos(mask_A)
    pos_B = first_error_pos(mask_B)
    ...
    candidates = [
        ("input", pos_input, noisy_img, noisy_seq),
        ("A",     pos_A,     arrA,      seqA),
        ("B",     pos_B,     arrB,      seqB)]
